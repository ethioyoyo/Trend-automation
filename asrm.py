...
params = {
    "top": 100   # REQUIRED for most ASRM endpoints
}

headers = {
    "Authorization": f"Bearer {TOKEN}",
    "Content-Type": "application/json",
    "TMV1-Filter": "(latestRiskScore ge 30)"
}
...
print(f"{'IP ADDRESS':<16} {'DEVICE NAME':<28} {'RISK':<6} {'CVE COUNT':<10}")
print("-" * 55)
...
#    print(device)
    ...
    ips = device.get("ip", [])
    risk = device.get("latestRiskScore", 0)
    cves = device.get("cveCount", 0)

    for ip in ips:
        if ip.startswith("fe80"):
            continue
        print(f"{ip:<16} {name:<28} {risk:<6} {cves:<6}")